...
func _ready():
	...
	match player.sprite.flip_h:
		true:
			angle = Vector2.LEFT
			position.x = player.global_position.x -25
	...
	match player.sprite.flip_h:
		true:
			...
			if delt<36:
				...
				self.position.x = player.global_position.x -25 - delt*0.194444 - 5*(attack_size-1)
			else:
				self.position.y = player.global_position.y + 11
				self.position.x = player.global_position.x -32 - 5*(attack_size-1)
